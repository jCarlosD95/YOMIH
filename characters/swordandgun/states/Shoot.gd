...
func _frame_3():
	if not temporal:
		...
		var shot_angle = fixed.vec_to_angle(fixed.mul(str(host.shot_dir_x), str(host.get_facing_int())), str(host.shot_dir_y)) if not auto else angle
		...
		if host.opponent.is_in_group("Cowboy"):
			...
			if opponent_state.name == "Shoot" and not opponent_state.temporal and opponent_state.start_tick == start_tick:
...
func _tick():
	...
	if air_type == AirType.Aerial:
		if current_tick > 2 and host.is_grounded():
			return "Landing"
	if current_tick == 7 and not temporal:
		if host.consecutive_shots > 0:
			if host.bullets_left > 0:
				return "Shoot"
	if parried and not dodge:

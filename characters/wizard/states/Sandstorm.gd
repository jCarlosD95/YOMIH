...
func _tick():
	...
	if current_tick < iasa_at and current_tick > 1:
		...
		for obj in host.objs_map.values():
			if obj is BaseObj and not obj == host and (obj.current_state() and obj.current_state().state_name != "Burst" and wb.overlaps(obj.hurtbox)):
		...
		host.apply_force(fixed.mul(pushback_dir.x, PUSHBACK_SPEED_X), fixed.mul(pushback_dir.y, PUSHBACK_SPEED_Y) if ( not host.is_grounded() or host.hovering) else "0")

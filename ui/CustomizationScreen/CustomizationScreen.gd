...
func init():
	for name in Global.name_paths:
		if (name in Global.paid_characters) and not Global.full_version():
			continue
	...
	if not SteamHustle.WORKSHOP_ENABLED:
...
func save_style(clear_text = false):
	...
	if data.character_color == Color("0b0c0f"):
		if not data.use_outline or data.outline_color == Color("0b0c0f"):

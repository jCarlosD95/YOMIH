...
func _physics_process(_delta):
	if is_instance_valid(game):
		...
		if p1.penalty_ticks <= 0:
			p1_sadness_label.visible = p1_sadness_label.visible and Utils.wave(0, 1, 0.25) < 0.5
		...
		if p2.penalty_ticks <= 0:
			p2_sadness_label.visible = p2_sadness_label.visible and Utils.wave(0, 1, 0.25) < 0.5
